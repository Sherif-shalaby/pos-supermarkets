                                        ...
                                        <tr class="product_row">
                                            ...
                                            <td>
                                                <input type="text"
                                                    ...
                                                    @if(isset($product->product->units) &&
                                                count($product->product->units) > 0)
                                                min="{{$product->product->units->pluck('name')[0]==("piece"||"Piece"||"قطعة"
                                                || "قطعه" || "") ? 1 : 0.00001}}"
                                                @else
                                                min="0.00001"
                                                @endif
                                                name="add_stock_lines[{{$loop->index}}][quantity]" required
                                                value="@if(isset($product->quantity)){{preg_match('/\.\d*[1-9]+/',
                                                (string)$product->quantity) ? $product->quantity :
                                                @num_format($product->quantity)}}@else{{1}}@endif"
                                                index_id="{{$loop->index}}">
                                            </td>
                                            <td>
